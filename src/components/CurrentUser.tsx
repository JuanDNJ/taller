...
const CurrentUser = () => {
    ...
    return (
        <section className="relative flex gap-4 items-center justify-between rounded-md py-1 px-2 my-1 mr-4">
            {user ? <>
                <header className="flex items-center gap-2" onClick={() => setIsOpenUserMenu(!isOpenUserMenu)}>
                    {user.photoURL && <Avatar src={user.photoURL as string} alt={user.displayName || "User Avatar"} />}
                ...
                {isOpenUserMenu && (
                    <article className="absolute top-full -left-full sm:left-0 mt-2 bg-white rounded-lg shadow-lg p-4 flex flex-col gap-4 lg:mr-4">
                        <nav>
